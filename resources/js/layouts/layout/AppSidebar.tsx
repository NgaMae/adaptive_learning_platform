            ...
            {items.map((nav, index) => (
                <li key={nav.name}>
                    {nav.subItems ? (
                        ...
                        nav.path && (
                            <Link to={nav.path} className={`group menu-item ${isActive(nav.path) ? 'menu-item-active' : 'menu-item-inactive'}`}>
            ...
            <div className={`flex py-8 ${!isExpanded && !isHovered ? 'lg:justify-center' : 'justify-start'}`}>
                <Link to="/">
                    {isExpanded || isHovered || isMobileOpen ? (
                        ...
                        // </>
                        <div className="text-xl font-bold dark:text-white">AdaptiveLearningPlatform</div>
                    ) : (
